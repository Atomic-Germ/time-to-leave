...
describe('Preload.mjs', function()
{
    ...
    beforeEach(function()
    {
        ...
        // Read the preload.mjs file and convert ES modules to CommonJS for VM execution
        const preloadPath = join(__dirname, '../../js/preload.mjs');
        const originalCode = readFileSync(preloadPath, 'utf8');

        // Convert import statement to require for VM execution
        preloadCode = originalCode.replace(
    ...
    describe('ES Module Features', function()
    {
        it('should import electron APIs correctly', function()
        {
            // This test verifies that the import statement works as expected
            // by checking that the functionality is preserved in our converted code
            vm.runInContext(preloadCode, context);

            // If we get here without errors, the import was handled correctly
            const exposedApi = mockContextBridge._exposedAPI.api;
            assert.strictEqual(typeof exposedApi.getAvailableThemes, 'function', 'Should have imported contextBridge functionality');
            assert.strictEqual(typeof exposedApi.getAppPath, 'function', 'Should have imported ipcRenderer functionality');
        });
    });
});